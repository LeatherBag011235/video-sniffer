...
class SegmentLinks(TypedDict):
    """Type definition for segment links dictionary"""
    __annotations__ = {
        str: str  # key: segment number as string, value: URL
    }

class VideoSegmentGluer:
    def __init__(
        self,
        segment_links: dict[str, str] | SegmentLinks,
        save_dir: Path | str = Path('C:/Video_Sniffer/saved_videos/'),
            ...
            for attempt in range(self.max_retries + 1):
                try:
                    ...
                    if attempt < self.max_retries:
                        print(f"Retrying segment {filename.name} (attempt {attempt + 1}/{self.max_retries})")
                except Exception as e:
                    if attempt >= self.max_retries:
                        warnings.warn(f"Failed to download segment {filename.name} after {self.max_retries} retries: {str(e)}")
            return False
    ...
    def download_all_segments(self) -> list[Path]:
        ...
        if success_count != total_count:
            warnings.warn(f"Only downloaded {success_count}/{total_count} segments successfully")
        
        print(f"Download results: {success_count} succeeded, {total_count - success_count} failed")
    ...
    def combine_segments(self, segment_files: list[Path], output_filename: str = 'combined_video.ts') -> Path:
        ...
        print(f"Combining {len(segment_files)} segments into {output_path}")
